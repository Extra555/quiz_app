...
test('health, auth and quiz lifecycle', async (t) => {
  ...
  const response = await fetch(`${base}/quizzes`, { method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${auth.token}` }, body: JSON.stringify({ title: 'Тестовый квиз', questions: [{ prompt: '2 + 2?', answerType: 'single', options: [{ text: '4', correct: true }, { text: '5', correct: false }, { text: '', correct: false }, { text: '   ', correct: false }] }] }) })
