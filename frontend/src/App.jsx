...
function Shell({ page, user, onNavigate, onLogout, children }) {
  ...
  return <main className={`app-shell page-${page}`}>
    <div className="confetti" aria-hidden="true">
      {Array.from({ length: 30 }, (_, index) => <i key={index} style={{ '--x': `${(index * 37) % 101}%`, '--delay': `${-(index % 10) * 1.15}s`, '--duration': `${7 + (index % 6) * 1.4}s`, '--size': `${5 + (index % 4) * 3}px`, '--drift': `${-35 + (index % 8) * 10}px` }} />)}
    </div>
